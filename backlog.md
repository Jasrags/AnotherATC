...
- 🚧 Departure releases / wheels-up windows from TRACON — the **time** half is built and live (see Clearance Delivery: EDCT with the clearance, window enforced at the takeoff clearance). What still needs TRACON is the **release** itself — Tower asking for and being given permission to launch, one at a time, during a push _(deferred — needs TRACON)_. _Multi-runway note: the engine has **no single-departure-runway assumption** — occupancy and wake are per-runway (`blocksRunwayFor` scopes to the aircraft's own runway; `lastDepartureByRunway`), so two independent runways can have departures rolling at once (proven by `koakAirport.test.ts` "two departures on independent runways… can both be cleared and rolling"). When the release flow is built it is a **per-push / per-runway** queue, not a field-wide serialization._
