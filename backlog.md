...
- ⬜ **Name untagged taxiway segments** (route-builder data fix) — patch ~31 unnamed OSM segments to their real designators (gaps in the A/B/C spines + numbered connectors) in `tools/ingest/build-ksan-surface.mjs`; terminal-apron lanes stay unnamed. Fixes the through-taxiways that currently can't be clicked into a route (also improves `routeVia` + labels). Full mapping cross-referenced to the airport diagram in `docs/SAN/taxiway-naming.md`.
