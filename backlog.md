...
- 🚧 **Assigned taxi routes** — clearance as a sequence of named taxiways ("via B, C"). **Shipped:** graph edges carry designators; `routeVia` follows an ordered taxiway sequence (falls back to shortest path); `taxiVia` command; strips derive & display "VIA A · B · C" for every route. _Next: let the player **build/edit** the sequence (click taxiways on the scope, or chips) and issue it; readback confirmation._
- ⬜ **Player-instructed give-way / reroute** (depends on assigned taxi routes) — surface conflict resolution as a ground-control action: "give way to traffic", reroute via a taxiway sequence, hold/continue; the sim's automatic floor stays as the safety net.
