...
- ✅ Departure releases / wheels-up windows from TRACON — **both halves built.** The **time** half (EDCT with the clearance, window enforced at the takeoff clearance). The **release** half is now live (2026-07-24, docs/atc-departure-release.md): at a release field every departure is flagged **needs release** at clearance; Tower issues **Call TRACON for release** (a silent landline); TRACON grants it after `coordSec` and no more than one per `intervalSec` **per runway**; a takeoff without a current release is refused (*call for release* / *hold for release*), and a release lapses after `voidSec` if unused, sending that departure back to the metering queue — the cascade. KBUR carries a config (SoCal meters Burbank); KSAN/KOAK opt in later. The metering is genuinely **per-runway** (keyed by `lastReleaseByRunway`), so it composes with the no-single-departure-runway model: occupancy and wake are per-runway (`blocksRunwayFor` scopes to the aircraft's own runway; `lastDepartureByRunway`), two independent runways can have departures rolling at once (`koakAirport.test.ts`), and their releases are independent (`departureRelease.test.ts`). _Deferred until TRACON is a real position: the release as a player-side radar-handoff decision, and a workload-varying release **rate** rather than the fixed interval._
- ⬜ **Per-runway slot / EDCT policy** (follow-up to the operations designation) — the slot lead is a single **field-wide** number (`Airport.slots`), applied per departure. With two departure runways in use both draw from the same policy; there is no per-runway slot rate or shared departure-release queue. Fine today (the window is per-aircraft, so runways don't contend), but a real field can meter its runways differently — revisit if/when departure demand modelling wants it.
