...
## AnotherATC binding (overrides the generic tooling below)

This is a **pnpm workspace**, tested with **Vitest** — not Jest, and the app is **Vite + React**, not Next.js. Node ≥ 22.13 is required, so route commands through fnm.

- `<test>` → `fnm exec --using=22 -- pnpm -r test` (or `make test`); single sim file: `fnm exec --using=22 -- pnpm --filter @anotheratc/sim exec vitest run src/ground/<file>.test.ts`
- `<test-watch>` → `make watch` (dev server + `tsc --watch` + Vitest together)
- `<coverage>` → not yet wired (`make check` = typecheck + lint + test); treat 80% as the target, not a gate
- `<lint>` → `fnm exec --using=22 -- pnpm lint` (includes the sim headless-boundary ESLint rule)
- Test imports use `vitest` (`import { describe, it, expect } from 'vitest'`) — ignore all `jest.*`, `bun:test`, Supabase, and Next.js examples below; translate them to Vitest (`vi.fn()`, `vi.mock()`).

**Two test worlds:** `packages/sim` is the **headless deterministic core** — pure TS, no DOM/React, and never `Math.random()`/`Date.now()` (thread the seeded `Rng`); most tests live here and assert sim behavior over fixed `step(dt)` loops. `apps/web` is DOM/React (Canvas2D scope + strips) — prefer visual/interaction checks; it has no unit tests today. See `CLAUDE.md`.
