...
| One runway at a time | ✅ At KOAK — two independent parallels — a crossing clearance authorizes exactly one runway: on physically clearing it the aircraft re-holds short of the next runway its route reaches and needs a fresh clearance (`resolveRunwayCrossing` / `maybeReholdAtNextRunway`). Two shapes: a North-Field→terminal taxi *crosses* both parallels to a gate beyond (re-hold at each, handed back to Ground between); and a departure *crosses* one parallel to reach the OTHER as its **departure runway** (goal on it, e.g. cross 28R to depart 28L) — it re-holds short of that runway as a takeoff hold, kept by whoever ran the crossing (its own runway's Tower gives the line-up; Ground would hand it to Tower). Never a runway occupancy-coupled to the aircraft's own, so an *intersecting* field (KBUR) is unaffected — the coupling gates the shared intersection — and a single-runway field (KSAN) is a strict no-op. The occupancy that backs it is position-aware: an aircraft holds a runway only while physically on it (`occupiesForTakeoff`), and only *that* runway (`blocksRunwayFor`). |
